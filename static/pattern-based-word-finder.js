...
function importLanguageDictionary(lang) {
	if (lang == "custom") {
		...
		document.getElementById("lang-custom").style.background = "var(--wincol-lighter)";
	}
	else {
...
function addRule(e) {
	...
	rule.innerHTML = o.innerHTML.replace("<div class=\"add-pattern\" onclick=\"addRule(event)\" style=\"opacity: 0;\"><img src=\"../static-0/files/images/icons/add.svg\"><p>Click to add rule</p></div>", "");
...
function updatePattern() {
	...
	for (var i=0; i < patternContainer.children.length; i++) {
		...
		if (p.type == "contains" || p.type == "containsany" || p.type == "startswith" || p.type == "endswith" || p.type == "lengthis" || p.type == "lengthislessthan" || p.type == "lengthismorethan" || p.type == "hasnvowels" || p.type == "haslessthannvowels" || p.type == "hasmorethannvowels" || p.type == "hasnconsonants" || p.type == "haslessthannconsonants" || p.type == "hasmorethannconsonants") {
	...
	if (langLoaded == true) {
		...
		for (var i=0; i < dict.length; i++) {
			...
			for (var n=0; n < pattern.length; n++) {
				...
				else if (p.type == "containsany") {
					if (p.logic == "invert") {
						var e = 0;
						for (const c of p.val1.split("")) {if (w.indexOf(c) > -1) {e++}};
						if (e > 0) {valid = -100}
						else {valid += 1};
					}
					else {
						for (const c of p.val1.split("")) {if (w.indexOf(c) > -1) {valid += 1; break}};
					}
				}
				...
				else if (p.type == "doesnothavepunctuation") {
					if (p.logic == "invert") {
						var ww = w.split("");
						var m = 0;
						for (var t=0; t < ww.length; t++) {if (ww[t] == "!" || ww[t] == "&" || ww[t] == "'" || ww[t] == "," || ww[t] == "." || ww[t] == "/") {m++}};
						if (m > 0) {valid += 1}
						else {break};
					}
					else {
		...
		toolOutput.children[1].children[1].innerHTML = "Showing " + wordsWritten.toLocaleString() + "/" + words.length.toLocaleString() + " matching words";
...
function clearPattern() {
	if (patternContainer.children.length > 0) {
		if (patternContainer.children.length > 3 && confirm("Clear all rules? (you cannot undo)") == false) {return};
...
function setCustomSeparator(e) {
	...
	if (element.tagName == "INPUT") {
		element.parentNode.style.background = "var(--wincol-lighter)";
	}
	else {
		element.style.background = "var(--wincol-lighter)";
	}
