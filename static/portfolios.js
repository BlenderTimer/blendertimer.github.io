...
for (let c of collections) {
	if (c == "Featured Photos" && portfolioCollection == "Featured Photos") {portfolioCollections.innerHTML += `<a href="/portfolio" class="viewing-collection">Featured Photos</a>`}
	else if (c == "Featured Photos") {portfolioCollections.innerHTML += `<a href="/portfolio">Featured Photos</a>`}
	else if (portfolioCollection == c) {portfolioCollections.innerHTML += `<a href="/portfolio?c=${c.toLowerCase().replace(/ /g, "-")}" class="viewing-collection">${c}</a>`}
	else {portfolioCollections.innerHTML += `<a href="/portfolio?c=${c.toLowerCase().replace(/ /g, "-")}">${c}</a>`}
}
let leftScroll = 0;
						...
						if (contentViewing.length > 0 && photo.title.toLowerCase() == contentViewing) {
							...
							if (photo.location && photo.location.length > 0) {
								...
								if (photo.location.indexOf("http") > -1) {
									html += `<a class="info-location" href="${photo.location.removeBefore("|", 1).removeAfter("|", -1, true)}" title="${photo.location.removeBefore("|", 1, true)}" target="_blank"><img src="/static/images/icons/location.svg" alt="Location"><b>${photo.location.removeAfter("|", -1)}</b></a>`;
								}
								else {
									html += `<div class="info-location"><img src="/static/images/icons/location.svg" alt="Location"><b>${photo.location}</b></div>`;;
								}
							}
